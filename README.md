...
Preconfigured commands are available to interact with the application. The following commands assume the use of the provided `Makefile`.

| Command          | Description                                   |
| ---------------- | --------------------------------------------- |
| `make run-tools` | Run the tools specified in `tools/tools.go`.  |
| `make format`    | Format the code using `gofumpt`.              |
| `make tidy`      | Clean up the `go.mod` and `go.sum` files.     |
| `make run`       | Tidy up dependencies and run the application. |
| `make test`      | Tidy up dependencies and run all tests.       |
